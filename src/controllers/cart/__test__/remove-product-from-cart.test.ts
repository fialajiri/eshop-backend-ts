...
it("returns a 404 if the product does not exists", async () => {
  ...
  await request(app)
    .put(`/api/cart/removefromcart/${cart.id}`)
    .send({
...
it("returns a 404 if the cart does not exists", async () => {
  ...
  await request(app)
    .put(`/api/cart/removefromcart/${nonExistentCartId}`)
    .send({
      productId: product1.id,
    })
    .expect(404);
...
it("removes the product from the cart and restores the availability", async () => {
